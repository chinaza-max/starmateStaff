    ...
    <div className='Message'>
      ...
      <Paper square sx={{ pb: '50px' }}>
        <Typography variant="p" gutterBottom component="div" sx={{ p: 2, pb: 0,display: 'flex', gap: 1.5, '& > h5': { flex: 1 }  }}>
            ...
            <Typography  onClick={onClear} variant="h5" sx={{ textAlign: "right",color:"#e43e3e" }}>Clear</Typography>
        </Typography>
        <List sx={{ mb: 2 }}>
          ...
          {messages.map(({ id, primary, secondary, person ,date}) => (
            <React.Fragment key={id}>
              {isDateToday(new Date(date))===true && (
                <ListSubheader sx={{ bgcolor: 'background.paper' }}>
                  Today
                </ListSubheader>
              )}

              {isDateYesterday(new Date(date)) === true && (
                <ListSubheader sx={{ bgcolor: 'background.paper' }}>
                  Yesterday
                </ListSubheader>
              )}
              ...
              {isDateBeforeYesterday(new Date(date)) === true && (
                <ListSubheader sx={{ bgcolor: 'background.paper' }}>
                   {formatDateInWords(new Date(date))}
                </ListSubheader>
              )}
